...
void *Oxigenio(void *v) {
  ...
  if (numero_o >= 1 && numero_h >= 2) {
    ...
    pthread_mutex_unlock(&lock); // Liberando acesso às variaveis
    sem_wait(&sem_o); // Como o semaforo é iniciado em 0, todo oxigenio que
                      // chega aqui trava
...
void *Hidrogenio(void *v) {
  ...
  if (numero_o >= 1 && numero_h >= 2) {
    ...
    sem_post(&sem_o); // Liberamos um oxigenio e um hidrogenio (somados com mais
                      // esse hidrogenio que está executando)
    sem_post(&sem_h);
    ...
    pthread_mutex_unlock(&lock); // Liberando acesso às variaveis
    sem_wait(&sem_h); // Como o semaforo é iniciado em 0, todo oxigenio que
                      // chega aqui trava
